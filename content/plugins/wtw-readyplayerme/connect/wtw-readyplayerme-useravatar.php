...
try {
	...
	$wtwconnect->trackPageView($wtwconnect->domainurl."/connect/wtw-readyplayerme-useravatar.php");
	...
	$zresults = $wtwconnect->query("
		select * 
		from ".WTW_READYPLAYERME_PREFIX."contentratings
	...
	if ($wtwconnect->hasValue($zglobalhash)) {
		...
		$zresults = $wtwconnect->query("
			select * 
			from ".WTW_READYPLAYERME_PREFIX."useravatars 
		...
		foreach ($zresults as $zrow) {
			...
			$zresults2 = $wtwconnect->query("
				select * 
				from ".WTW_READYPLAYERME_PREFIX."users 
				where userid='".$zuserid."'
					and deleted=0
				limit 1;");
	...
	if ($wtwconnect->hasValue($zinstanceid)) {
		...
		$zresults = $wtwconnect->query("show tables like '".WTW_READYPLAYERME_PREFIX."3dinternet_blockedinstances';");
		...
		if ($zfoundbantable) {
			...
			$zresults = $wtwconnect->query("
				select *
				from ".WTW_READYPLAYERME_PREFIX."3dinternet_blockedinstances
	...
	$wtwconnect->serror("plugins:wtw-readyplayerme:connect-wtw-readyplayerme-useravatar.php=".$e->getMessage());
}
?>
