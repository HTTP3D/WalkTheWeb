...
class wtwshopping {
	...
	public function initAdminOnlyHooks() {
		...
		try {
			...
			if ($wtwplugins->pagename == "admin.php") {
				...
				$wtwplugins->addScriptFunction("setnewmold", "WTWShopping.setNewMold(moldname, molds, moldind, rebuildmold);");
				$wtwplugins->addScriptFunction("openaddnewmold", "WTWShopping.openAddNewMold(moldgroup, shape, moldname);");
				$wtwplugins->addScriptFunction("setnewmolddefaults", "WTWShopping.setNewMoldDefaults(shape);");
				$wtwplugins->addScriptFunction("setmoldformfields", "WTWShopping.setMoldFormFields(shape);");
				$wtwplugins->addScriptFunction("openmoldform", "WTWShopping.openMoldForm(moldname, molds, moldind, shape);");
				$wtwplugins->addScriptFunction("loadmoldform", "WTWShopping.loadMoldForm(moldgroup, shape, moldname);");
				$wtwplugins->addScriptFunction("submitmoldform", "WTWShopping.submitMoldForm(w);");
	...
	public function initHooks() {
		...
		try {
			...
			$wtwplugins->addScriptFunction("checkhovers", "WTWShopping.checkHovers(moldname, shape);");
			$wtwplugins->addScriptFunction("resethovers", "WTWShopping.resetHovers(moldname, shape);");
			$wtwplugins->addScriptFunction("opencolorselector", "WTWShopping.openColorSelector(moldname, shape, colortype);");

			$wtwplugins->addScriptFunction("moldqueueadd", "WTWShopping.moldQueueAdd(moldname, mold);");
			...
			$wtwplugins->addMoldDef("Store Product", "webmold", "WTWShopping.addMoldStoreProduct(moldname, molddef, lenx, leny, lenz);");

			$wtwplugins->addMoldDef("Store Add to Cart", "webmold", "WTWShopping.addMoldStoreButton(moldname, molddef, lenx, leny, lenz);");
			$wtwplugins->addMoldDef("Store Buy Now", "webmold", "WTWShopping.addMoldStoreButton(moldname, molddef, lenx, leny, lenz);");
			$wtwplugins->addMoldDef("Store Checkout", "webmold", "WTWShopping.addMoldStoreButton(moldname, molddef, lenx, leny, lenz);");
			$wtwplugins->addMoldDef("Store Read More", "webmold", "WTWShopping.addMoldStoreButton(moldname, molddef, lenx, leny, lenz);");

			$wtwplugins->addMoldDef("Store Sign", "webmold", "WTWShopping.addMoldStoreSign(moldname, molddef, lenx, leny, lenz);");
			$wtwplugins->addMoldDef("Store 3D Sign", "webmold", "WTWShopping.addMoldStore3DSign(moldname, molddef, lenx, leny, lenz);");
			$wtwplugins->addMoldDef("Store View Cart", "webmold", "WTWShopping.addMoldStoreViewCart(moldname, molddef, lenx, leny, lenz);");
			$wtwplugins->addMoldDef("Store Categories", "webmold", "WTWShopping.addMoldStoreCategories(moldname, molddef, lenx, leny, lenz);");
			$wtwplugins->addMoldDef("Store Search", "webmold", "WTWShopping.addMoldStoreSearch(moldname, molddef, lenx, leny, lenz);");
	...
	public function storeSettingsForm($zmoldgroup) {
		/* admin store settings form */
		global $wtwplugins;
		$zformdata = "";
		try {
			$zformdata .= "<h2 style=\"margin-bottom:3px;\">Connect 3D Store</h2><br /><div class=\"wtw-indent\">\r\n";
			$zformdata .= "<select id=\"wtwshopping_".$zmoldgroup."connectstore\"></select></div><br /><br />\r\n";
			$zformdata .= "<div id=\"wtwshopping_".$zmoldgroup."saveconnectstore\" class=\"wtw-greenbutton\" onclick=\"WTWShopping.saveConnectStore('".$zmoldgroup."');\">Save</div>\r\n";
		} catch (Exception $e) {
